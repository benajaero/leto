      ...
      <div className="flex items-center justify-around">
        {tabs.map((tab) => (
          ...
          >
            ...
            <span className="text-micro font-bold uppercase tracking-wider">{tab.label}</span>
          </button>
        ))}
      </div>
    </nav>
...
function ListIcon({ className, active }: { className?: string; active?: boolean }) {
  return (
    <svg className={className} fill="none" viewBox="0 0 24 24" stroke={active ? '#c9a86c' : 'currentColor'} strokeWidth={1.5}>
