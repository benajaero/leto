  ...
  return (
    <div className="relative flex h-full flex-col border-l border-r border-aerospace-700 bg-aerospace-950">
      <div className="flex items-center justify-between border-b border-aerospace-700 px-4 py-2">
        <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-aerospace-400">Tactical Display</span>
        <div className="flex items-center gap-3 text-[9px] text-aerospace-500">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full">
          ...
          {hLines.map((line, i) => (
            <g key={`h-${i}`}>
              ...
              <text x={padding - 4} y={line.y + 3} fill="#5a5a7a" fontSize="8" textAnchor="end" fontFamily="var(--font-jetbrains)">
                {line.label}
              </text>
            </g>
          ))}
          {vLines.map((line, i) => (
            <g key={`v-${i}`}>
              ...
              <text x={line.x} y={height - padding + 12} fill="#5a5a7a" fontSize="8" textAnchor="middle" fontFamily="var(--font-jetbrains)">
                {line.label}
              </text>
            </g>
          ))}
          ...
          <rect
            ...
            strokeDasharray="4 2"
          />
          <text x={aoiRect.x + 4} y={aoiRect.y - 4} fill="#00d4ff" fontSize="9" fontFamily="var(--font-jetbrains)">
            AOI
          </text>
          ...
          {scenario.stations.map((station) => {
            ...
            return (
              <g key={station.id}>
                <circle cx={pos.x} cy={pos.y} r="4" fill="#0a0a0f" stroke="#10b981" strokeWidth="1.5" />
                <circle cx={pos.x} cy={pos.y} r="12" fill="none" stroke="#10b981" strokeWidth="0.5" opacity="0.3" />
                <text x={pos.x + 8} y={pos.y + 3} fill="#10b981" fontSize="8" fontFamily="var(--font-jetbrains)">
                  {station.name}
                </text>
              </g>
            );
          ...
          {incidents.map((incident) => {
            ...
            return (
              ...
              >
                {isSelected && (
                  <circle cx={pos.x} cy={pos.y} r="14" fill="none" stroke="#00d4ff" strokeWidth="1" opacity="0.5">
                ...
                <circle
                  ...
                  r={isSelected ? 5 : 4}
                  fill={typeColor}
                  stroke={isSelected ? '#00d4ff' : '#0a0a0f'}
                  strokeWidth={isSelected ? 2 : 1}
                  opacity={hasCoverage ? 1 : 0.4}
                />
                {isSelected && (
                  <text x={pos.x + 8} y={pos.y - 4} fill="#e2e2f0" fontSize="8" fontFamily="var(--font-jetbrains)">
                    {incident.label}
                  </text>
                )}
              </g>
