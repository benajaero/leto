...
const SOURCES = [
  ...
  { name: 'GDACS', icon: '🌊', color: 'text-signal-400' },
  ...
  return (
    <footer className="flex items-center justify-between border-t border-aerospace-700 bg-aerospace-900/80 px-4 py-1.5 backdrop-blur">
      <div className="flex items-center gap-3">
        <span className="text-micro font-bold uppercase tracking-wider text-aerospace-500">Data Sources</span>
          ...
          {SOURCES.map((s) => {
            ...
            return (
              <div key={s.name} className="flex items-center gap-1 rounded border border-aerospace-700 bg-aerospace-800 px-1.5 py-0.5">
                <span className="text-readout">{s.icon}</span>
                <span className={`text-micro font-semibold ${active ? s.color : 'text-aerospace-500'}`}>{s.name}</span>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-micro font-semibold text-emerald-400">All Systems Operational</span>
        </div>
        <span className="text-micro text-aerospace-500">Data latency: 2–5 min</span>
        <span className="font-mono text-micro text-aerospace-500">v2.3.0</span>
      </div>
    </footer>
  );
}
